...
int main(){
    ...
    // Original heads for detecting full cycles in circular lists during merge
    ...
    // Make List 1 circular
    if(head1 != NULL){
        temp1->next = head1;
        original_head1 = head1; // Store original head
    }
    ...
    if(head1 == NULL){
        ...
        temp1 = head1; // Reset temp1 for printing traversal
        do {
            printf("%d\n",temp1->data);
            temp1 = temp1->next;
        } while(temp1 != head1);
    ...
    // Make List 2 circular
    if(head2 != NULL){
        temp2->next = head2;
        original_head2 = head2; // Store original head
    }
    ...
    if(head2 == NULL){
        ...
        temp2 = head2; // Reset temp2 for printing traversal
        do {
            printf("%d\n",temp2->data);
            temp2 = temp2->next;
        } while(temp2 != head2);
    ...
    // Reset temp1 and temp2 to original heads for merge logic
    ...
    if (temp1==NULL){
        ...
        if(rhead != NULL){ // Only make circular if the merged list is not empty
            rtemp->next = rhead;
        }
    }
    ...
    if(rhead == NULL){
        ...
        struct node *temp_print_merged = rhead; // Use a new temp for printing merged list
        do {
    ...
    if(rhead != NULL){
        ...
        last_node_free->next = NULL; // Break the circle
